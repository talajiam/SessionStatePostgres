...
namespace SessionState.Postgres
{
    public class SqlSessionStateProviderAsync : SessionStateStoreProviderAsyncBase
    {
        ...
        private static IPostgresSessionStateRepository s_sqlSessionStateRepository;
        ...
        internal IPostgresSessionStateRepository SqlSessionStateRepository
        {
        ...
        internal static Func<HttpContext, HttpStaticObjectsCollection> GetSessionStaticObjects { get; set; } =
            ...
            CancellationToken cancellationToken)
        {
            ...
            await s_sqlSessionStateRepository.CreateOrUpdateSessionStateItemAsync(newItem, id, buf, length,
                item.Timeout, lockCookie, _rqOrigStreamLen);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                return new GetItemResult(null, sessionItem.Locked, sessionItem.LockAge, sessionItem.LockId,
                    sessionItem.Actions);
            }
        ...
        internal static SessionStateStoreData DeserializeStoreData(HttpContextBase context, Stream stream,
            bool compressionEnabled)
        {
            if (compressionEnabled)
            {
        ...
        private static ConnectionStringSettings GetConnectionString(string connectionstringName)
        {
            ...
            {
                throw new ProviderException(Resource1.Connection_name_not_specified);
            }
            ConnectionStringSettings conn = ConfigurationManager.ConnectionStrings[connectionstringName];
            if (conn == null)
            {
                throw new ProviderException(
                    String.Format(CultureInfo.CurrentCulture, Resource1.Connection_string_not_found, connectionstringName));
            }
            return conn;
        }
    }
